...
Class Order_model extends MY_Model {
	...
	function getFilterOrders($filterData=array()){
		try{
			...
			$sql		= "select * from qw.get_all_orders_by_date('$orderStatus','$shipDateFrom','$shipDateTo','$shippMethode','$shippLocation','$refNum')";
			$results	= $this->db->query($sql);
			$results	= $results->row();
			if(!empty($results)){
                ...
                function updateSalesOrderInfo($saleOrderInfo=array()){
                     try{
                                                ...
                                                if(!empty($login_user) && !empty($saleOrderInfo)){
                                                                ...
                                                                $sql                        = "select * from qw.save_salesorder_details('$salesOrderNum','$salesOrderLinekey','$salesOrderWtStep',$salesOrderTimeTaken,$salesOrderUsedQnty,'$salesOrderComments','$salesOrderJobStatus','$login_user')";
                                                                $results = $this->db->query($sql);
                                                                $results = $results->row();
                                                                if(!empty($results)){
