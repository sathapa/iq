...
class Login_model extends MY_Model {
	...
	/**
	 * This function used for checked that Frontend (salesmen) user login or not.
	 * 
	...
	public function loggedIn(){
		...
		$email		= $this->session->userdata('frontendUserEmail');
	...
	public function loggedOut(){
		$newdata	= array('frontendUserName'=>'','frontendUserEmail'=>'');
	...
	public function loggedInUser(){
		try{
			...
			$email		= $this->session->userdata('frontendUserEmail');
			if(!empty($userName) && !empty($email)){
				$user = $this->db->where('username',$userName)->where('email',$email)->get('qw.qw_users')->row();
				if($user){
					return $user;
				}else{
					return false;
	...
	/**
	 * This function used for frontend user login
	 * 
	...
	public function login($email=null,$password=null,$remeber=NULL){
		try{
			if(!empty($email) && !empty($password)){
				...
				$user		= $this->db->where('email',$email)->where('password',$password)->get('qw.qw_users')->row();
					if($user){
						$this->session->set_userdata('frontendUserName',$user->username);
						$this->session->set_userdata('frontendUserEmail',$user->email);
						$this->session->set_userdata('frontendUserId',$user->id);
						$this->session->set_userdata('frontendUserGroup',$user->user_group_id);
						$this->session->set_userdata('images',$user->image);
	...
	function resetPassword($userId=null,$password=null){
		try{
			if(!empty($userId) && !empty($password)){
				$update	= $this->db->where('id',$userId)->update('qw.qw_users',array('password'=>$password));
				if($update){
