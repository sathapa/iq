	...
	<div class="page-header">
		<h1>
			...
			<?php
				...
				$about		= !empty($user->aboutus) ? $user->aboutus : '';
				$active		= !empty($user->active) ? $user->active : 0;
				$image		= !empty($user->image) ? $user->image : '';
				$group		= !empty($user->user_group) ? $user->user_group : '';
			?>
		</h1>
	</div><!-- /.page-header -->
		...
		<div class="col-xs-12">
			...
			<?=form_open(current_url(),array('id'=>'user-register','enctype'=>'multipart/form-data','class'=>'form-horizontal','data-parsley-validate'=>'data-parsley-validate'))?>
							...
							<?php
								...
								if(!empty($groups)){
									foreach($groups as $k=>$val){
										$sell	= (!empty($group) && $group==$k )? 'selected' :'' ;
										echo '<option value="'.$k.'" '.$sell.'>'.$val.'</option>';
									}
								}
							?>
						</select>
				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						<input type="text" name="email" class="col-xs-10 col-sm-5" placeholder="Email Address" id="email" data-parsley-required-message="Please Enter Email Id" data-parsley-required="true" 
							data-parsley-type="email" data-parsley-type-message="Please Enter Valid Email Id" value="<?=$email?>">
					</div>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						<label>
							<input  name="status" value="1" class="ace" type="radio" <?=!empty($active) ? 'checked' : '';?>>
							<span class="lbl"> Active</span>
						</label>
						<label>
							<input  name="status" value="0" class="ace" type="radio" <?=empty($active) ? 'checked' : '';?>>
